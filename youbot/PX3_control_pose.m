...
while true,
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if strcmp(fsm, 'rotate'),
		...
		angl;
	    rotVel =2*angl-1*(-youbotEuler(3)-angl+target(3));

        fsm = 'drive';
		if abs(angdiff(target(3), youbotEuler(3))) < 1/180*pi,
            rotVel = 0;
        end
% 		forwBackVel=0;
...
%         
% 		 rotVel=0;
         if sqrt((youbotPos(1)-target(1))^2+(youbotPos(2)-target(2))^2)<0.05
             fsm='finished';
         end
