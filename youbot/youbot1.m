...
% (See http://www.gnu.org/copyleft/gpl.html)
   ...
   % Make sure everything is settled before we start
    pause(2);
    [res blueCylinder] = vrep.simxGetObjectPosition(h.id, h.blueCylinder, -1,vrep.simx_opmode_oneshot_wait);
	vrchk(vrep, res);
	[res yellowRectangle] = vrep.simxGetObjectPosition(h.id, h.yellowRectangle,-1,vrep.simx_opmode_oneshot_wait);
	vrchk(vrep, res);
	[res blueCylinder_O] = vrep.simxGetObjectOrientation(h.id, h.blueCylinder, -1,vrep.simx_opmode_oneshot_wait);
	vrchk(vrep, res);
	[res yellowRectangle_O] = vrep.simxGetObjectOrientation(h.id, h.yellowRectangle, -1,vrep.simx_opmode_oneshot_wait); 
	vrchk(vrep, res);
...
%             end
   ...
   elseif strcmp(fsm, 'snapshot'),
        ...
        % Reading a 3D image costs a lot to VREP (vrep has to simulate the image). Italso requires a lot of bandwidth, and processing a 3D point cloud (for
        % instance, to find one of the boxes or cylinders that the robot has to grasp) will take a long time in Matlab. In general, you will only want to
        % capture a 3D image at specific times, for instance when you believe you're facing one of the tables.
        ...
        % Each column of pts has [x;y;z;distancetosensor],Here, we only keep points within 1 meter, to focus on the table
        pts = pts(1:3,pts(4,:)<1);
        subplot(223)
        ...
        % Read data from the RGB camera This is very similar to reading from the 3D camera. The comments in the 3Dcamera section directly apply to this section.
   ...
   elseif strcmp(fsm, 'reachout'),
        ...
        tpos(1) = tpos(1)+.01;
        res = vrep.simxSetObjectPosition(id, h.ptarget, -1, yellowRectangle,...
                                    vrep.simx_opmode_oneshot);
        vrchk(vrep, res, true);
        disp('reachout');
